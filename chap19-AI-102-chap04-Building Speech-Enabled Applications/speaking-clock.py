...
def main():
    try:
        ...
        speech_config = speech_sdk.SpeechConfig(cog_key, cog_region, speech_recognition_language='en-US') # en-US zh-TW
        print('Ready to use speech service in:', speech_config.region) 

        # Get spoken input
        command = TranscribeCommand()
        if command.lower() in['what time is it?', 'what time?', '幾點？', 'what time？','what time is it？']:
            TellTime()
        else:
            print('尚未抓到')
